...
const PuppyBuddy = ({ celebrateKey, onActivate }: PuppyBuddyProps) => {
  ...
  return (
    <div className="character-buddy-wrapper" aria-live="polite" aria-atomic="true">
      ...
      <button
        type="button"
        className={`character-click-target puppy-character ${isCheering ? "puppy-cheer" : ""}`}
      ...
      >
        <svg viewBox="0 0 180 180" className="puppy-svg" role="img" aria-hidden="true">
          ...
          <circle className={`puppy-eye ${isCheering ? "puppy-eye-happy" : ""}`} cx="72" cy="86" r="7" fill="#332016" />
          <circle className={`puppy-eye ${isCheering ? "puppy-eye-happy" : ""}`} cx="118" cy="86" r="7" fill="#332016" />
          <circle className="puppy-eye-sparkle" cx="74" cy="83" r="2" fill="#fff" />
          <circle className="puppy-eye-sparkle" cx="120" cy="83" r="2" fill="#fff" />
          <path
            className={`puppy-smile ${isCheering ? "puppy-smile-happy" : ""}`}
            d={isCheering ? "M80 121 Q95 142 110 121" : "M82 123 Q95 133 108 123"}
            fill="none"
            stroke="#5a2f19"
